...
@app.get("/rooms")
def list_rooms(topic_id: str, status: str = "waiting"):
    """Used by the Mini App to check if a debate room already exists for a topic."""
    rooms = list(rooms_col.find({"topic_id": topic_id, "status": status}))
    for r in rooms:
        r["id"] = str(r.pop("_id"))
    return rooms
