...
@app.post("/users/{telegram_id}/chat-background")
def set_chat_background(telegram_id: int, body: ChatBackgroundIn):
    user = users_col.find_one({"telegram_id": telegram_id})
    if not user:
        raise HTTPException(404, "User not found")

    if body.other_id:
        key = f"chat_backgrounds.{body.other_id}"
        if body.background:
            users_col.update_one({"telegram_id": telegram_id}, {"$set": {key: body.background}})
        else:
            users_col.update_one({"telegram_id": telegram_id}, {"$unset": {key: ""}})
    else:
        users_col.update_one({"telegram_id": telegram_id}, {"$set": {"chat_background": body.background}})
    return {"ok": True}
