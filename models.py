...
class UserSettingsIn(BaseModel):
    online_visibility: Optional[Literal["everyone", "friends", "no_one"]] = None
    language: Optional[Literal["en", "hi"]] = None


class ChatBackgroundIn(BaseModel):
    background: Optional[str] = None  # None clears it back to default


# ---------- BACKGROUND SHOP (admin catalog, unlocked with coins) ----------
class BackgroundShopItemIn(BaseModel):
    name: str
    image: str  # base64 image
    price: int = Field(ge=0)


class PurchaseBackgroundIn(BaseModel):
    telegram_id: int
    item_id: str


class ApplyBackgroundIn(BaseModel):
    telegram_id: int
    item_id: Optional[str] = None  # None = revert to default (no background)
